...
pub struct AppData {
    /// Buffer for reading from/writing to the FIFO.
    fifo_buffer: ReadWriteProcessBuffer,
...
impl Default for AppData {
    fn default() -> AppData {
        AppData {
            fifo_buffer: ReadWriteProcessBuffer::default(),
...
impl<A: 'static + time::Frequency, B: 'static + time::Ticks> RFM69<A, B> {
...

    {
        ...
        // Configure SPI.
        spi.configure(spi::ClockPolarity::IdleLow, spi::ClockPhase::SampleLeading, 1_000_000)
            .unwrap();
        ...
        if let Some(o) = self.pending.iter().find(|op| op.get().is_some()) {
            ...
            match operation {
                ...
                Operation::FIFOWrite => {
                    ...
                    let grant_result = self.grants.enter(pid, |app_data, _upcall_table| {
                        app_data.fifo_buffer
                            .enter(|ro_slice| {
    ...
    fn process_callback(&self) -> Result<()> {
        ...
        let update_operation = match current_operation.get().unwrap() {
            ...
            Operation::FIFORead => {
                ...
                let copy_result = self.grants.enter(rx_pid, |app_data, upcall_table| {
                    // Copy up to either the shared buffer's length or the FIFO size,
                    // whichever buffer is the smaller of the two.
                    let copy_len = core::cmp::min(app_data.fifo_buffer.len(), FIFO_LENGTH);
                    app_data.fifo_buffer.mut_enter(|rw_slice| {
                    ...
                    upcall_table.schedule_upcall(grant_nos::SUBSCRIBE_RX_COMPLETE, (copy_len, 0, 0)).unwrap();
                });
...
impl<A: 'static + time::Frequency, B: 'static + time::Ticks> SyscallDriver for RFM69<A, B> {
    ...
    ) -> core::result::Result<ReadWriteProcessBuffer, (ReadWriteProcessBuffer, ErrorCode)> {
        let res = match allow_no {
            grant_nos::ALLOW_RW_FIFO_BUFFER =>
                self.grants.enter(pid, |app_data, _upcall_table| {
                    core::mem::swap(&mut buffer, &mut app_data.fifo_buffer);
